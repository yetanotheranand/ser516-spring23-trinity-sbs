...
export class DragDropScrumBoardComponent {
  ...
  containers = [
    { id: 'toDo', name: 'To Do', status: 'To Do', userStories: this.toDo },
    {
      ...
      name: 'In Progress',
      status: 'In Progress',
      userStories: this.inProgress,
    },
    {
      id: 'testing',
      name: 'Testing',
      status: 'Testing',
      userStories: this.testing,
    },
    {
      id: 'waiting',
      name: 'Waiting',
      status: 'Waiting',
      userStories: this.waiting,
    },
  ...
  onDrop(event) {
    const previousContainer = this.containers.find(
      (c) => c.userStories === event.previousContainer.data
    );
    const currentContainer = this.containers.find(
      (c) => c.userStories === event.container.data
    );
