...
@Service("dnc")
public class DivideAndConquerSimulationStrategy implements SimulationStrategy {
    @Override
    ...
    private Optional<UserStory> getStory(String name, List<UserStory> userStories) {
        ...
        if (userStory.isEmpty()) {
            userStory = userStories.stream()
                    .filter(us -> us.getStatus() == Status.TODO && (Objects.isNull(
                            us.getAssignedTo()) || us.getAssignedTo().isBlank()))
                    .findFirst();
        }
        return userStory;
    }
